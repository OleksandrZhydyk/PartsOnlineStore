...
def index_view(request):
    ...
    return render(
        ...
        context={
            "title": "Main page",
            "data": coordinates,
            "google_maps_api_key": settings.dev.GOOGLE_MAPS_API_KEY,
...
def get_parts_view(request):
    ...
    models = MachineModel.objects.all()
        # machine_systems = data.distinct("machine_system")
        # machine_systems = data.values('machine_system')
...
def get_part_detail(request, **kwargs):
    ...
    return render(
        ...
        context={
            "title": "Part detail",
            "part": part, "shops": shops, "comments": comments})
